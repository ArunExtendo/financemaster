...
@RequestMapping("/mitemcatg")
public class MItemCatgController {
	
	@Autowired
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;
	private MItemCatgService entityService;
	
	private static final Logger logger = Logger.getLogger(MItemCatgController.class);
	
	@ApiOperation(value = "API to Create or Update Item catg Entity.", response = Response.class)
	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(
			@ApiParam(value = "Request payload") @Valid @RequestBody MItemCatg request,
	...
	@ApiOperation(value = "Allows to fetch a Item catg Entity with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<MItemCatg> dataObj = entityService.findByCodes(request.getCode()[0],request.getCode()[1]);
