...
@RequestMapping("/mGLAccountComp")
public class MGlAccountCompController {

	@Autowired
	private MGlAccountCompService entityService;
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;

	private static final Logger logger = Logger.getLogger(MGlAccountCompController.class);

	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(
			@ApiParam(value = "Request payload") @Valid @RequestBody MGlAccountComp request,
	...
	@ApiOperation(value = "Allows to fetch Gl account comp Grid List.", response = Response.class)
	...
	public ResponseEntity<?> getAll(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		TransactionContext context = responseGenerator.generateTransationContext(httpHeader);
		try {
			List<MGlAccountComp> obj = new ArrayList<>();
			Page<MGlAccountComp> list = entityService.findAllMGlAccountCompDetails(request);
			obj = list.getContent();
	...
	@ApiOperation(value = "Allows to fetch a Gl account Entity with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<MGlAccountComp> dataObj = entityService.findById(request.getCode()[0]);
