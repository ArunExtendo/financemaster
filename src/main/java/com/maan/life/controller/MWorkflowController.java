...
@RequestMapping("/mworkflow")
public class MWorkflowController {

	@Autowired
	private MWorkflowService entityService;
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;

	private static final Logger logger = Logger.getLogger(MWorkflowController.class);

	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(@ApiParam(value = "Request payload") @Valid @RequestBody MWorkflow request,
	...
	@ApiOperation(value = "Allows to fetch WorkFlow Grid List.", response = Response.class)
	...
	public ResponseEntity<?> getAll(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		TransactionContext context = responseGenerator.generateTransationContext(httpHeader);
		try {
			List<MWorkflow> obj = new ArrayList<>();
			Page<MWorkflow> list = entityService.findAllWorkflowDetails(request);
			obj = list.getContent();
	...
	@ApiOperation(value = "Allows to fetch a WorkFlow with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<MWorkflow> dataObj = entityService.findById(request.getCode()[0]);
