...
@RequestMapping("/custRiskCalc")
public class WCustRiskCalcController {

	@Autowired
	private WCustRiskCalcService entityService;
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;

	private static final Logger logger = Logger.getLogger(WCustRiskCalcController.class);

	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(
			@ApiParam(value = "Request payload") @Valid @RequestBody WCustRiskCalc request,
	...
	@ApiOperation(value = "Allows to fetch Cust risk cal Grid List.", response = Response.class)
	...
	public ResponseEntity<?> getAll(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		TransactionContext context = responseGenerator.generateTransationContext(httpHeader);
		try {
			List<WCustRiskCalc> obj = new ArrayList<>();
			Page<WCustRiskCalc> list = entityService.findAllWCustRiskCalcDetails(request);
			obj = list.getContent();
	...
	@ApiOperation(value = "Allows to fetch a Cust risk cal Entity with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<WCustRiskCalc> dataObj = entityService.findById(request.getCode()[0]);
