...
@RequestMapping("/mwfStepsApplRoles")
public class MWfStepsApplRolesController {

	@Autowired
	private MWfStepsApplRolesService entityService;
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;

	private static final Logger logger = Logger.getLogger(MWfStepsApplRolesController.class);

	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(
			@ApiParam(value = "Request payload") @Valid @RequestBody MWfStepsApplRoles request,
	...
	@ApiOperation(value = "Allows to fetch Wf Steps ApplRoles Grid List.", response = Response.class)
	...
	public ResponseEntity<?> getAll(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		TransactionContext context = responseGenerator.generateTransationContext(httpHeader);
		try {
			List<MWfStepsApplRoles> obj = new ArrayList<>();
			Page<MWfStepsApplRoles> list = entityService.findAllWfStepsApplRolesDetails(request);
			obj = list.getContent();
	...
	@ApiOperation(value = "Allows to fetch a Wf Steps ApplRoles  with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<MWfStepsApplRoles> dataObj = entityService.findById(request);
