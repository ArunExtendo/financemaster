...
@RequestMapping("/wCustBankDtl")
public class WCustBankDtlController {

	@Autowired
	private WCustBankDtlService entityService;
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;

	private static final Logger logger = Logger.getLogger(WCustBankDtlController.class);

	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(
			@ApiParam(value = "Request payload") @Valid @RequestBody WCustBankDtl request,
	...
	@ApiOperation(value = "Allows to fetch Customer Bank details Grid List.", response = Response.class)
	...
	public ResponseEntity<?> getAll(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		TransactionContext context = responseGenerator.generateTransationContext(httpHeader);
		try {
			List<WCustBankDtl> obj = new ArrayList<>();
			Page<WCustBankDtl> list = entityService.findAllCustBankDtlDetails(request);
			obj = list.getContent();
	...
	@ApiOperation(value = "Allows to fetch a Customer Bank details Entity with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<WCustBankDtl> dataObj = entityService.findById(request.getCode()[0]);
