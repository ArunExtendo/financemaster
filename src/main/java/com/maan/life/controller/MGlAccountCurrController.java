...
@RequestMapping("/mGLAccountCurr")
public class MGlAccountCurrController {

	@Autowired
	private MGlAccountCurrService entityService;
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;

	private static final Logger logger = Logger.getLogger(MGlAccountCurrController.class);

	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(
			@ApiParam(value = "Request payload") @Valid @RequestBody MGlAccountCurr request,
	...
	@ApiOperation(value = "Allows to fetch Gl Account Curr Grid List.", response = Response.class)
	...
	public ResponseEntity<?> getAll(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		TransactionContext context = responseGenerator.generateTransationContext(httpHeader);
		try {
			List<MGlAccountCurr> obj = new ArrayList<>();
			Page<MGlAccountCurr> list = entityService.findAllGlAccountCurrDetails(request);
			obj = list.getContent();
	...
	@ApiOperation(value = "Allows to fetch a Gl Account Curr Entity with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<MGlAccountCurr> dataObj = entityService.findById(request);
