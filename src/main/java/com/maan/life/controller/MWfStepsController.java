...
@RequestMapping("/mWFSteps")
public class MWfStepsController {

	@Autowired
	private MWfStepsService entityService;
	private MessagePropertyService messageSource;
	private @NonNull ResponseGenerator responseGenerator;

	private static final Logger logger = Logger.getLogger(MWfStepsController.class);

	@PostMapping(value = "/createOrUpdate", produces = "application/json")
	public ResponseEntity<?> createOrUpdate(@ApiParam(value = "Request payload") @Valid @RequestBody MWfSteps request,
	...
	@ApiOperation(value = "Allows to fetch Wf steps Grid List.", response = Response.class)
	...
	public ResponseEntity<?> getAll(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		TransactionContext context = responseGenerator.generateTransationContext(httpHeader);
		try {
			List<MWfSteps> obj = new ArrayList<>();
			Page<MWfSteps> list = entityService.findAllWfStepsDetails(request);
			obj = list.getContent();
	...
	@ApiOperation(value = "Allows to fetch a Wf steps Entity with given code(primary key).", response = Response.class)
	...
	public ResponseEntity<?> getById(@RequestBody ListViewParam request, @RequestHeader HttpHeaders httpHeader)
		...
		try {
			if (!ValidationUtil.isEmptyStringArray(request.getCode())) {
				Optional<MWfSteps> dataObj = entityService.findById(request);
