...
public interface MCurrencyRepository extends JpaRepository<MCurrency, String>, JpaSpecificationExecutor<MCurrency> {
	...
	@Query(value = "select curr_code as currCode, curr_name as currName, curr_bl_name as currBlName, curr_short_name as currShortName, curr_bl_short_name as currBlShortName, curr_freez_yn as currFreezYn, curr_freez_dt as currFreezDt from M_CURRENCY where upper(concat(concat(concat(concat(concat(concat(curr_code,curr_name),curr_bl_name),curr_short_name),curr_bl_short_name),curr_freez_yn),curr_freez_dt)) like Upper(:sear)", nativeQuery = true)
